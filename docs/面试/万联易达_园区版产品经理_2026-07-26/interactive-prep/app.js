const STORAGE_KEY = "wanlianyida_park_pm_prep_cn_v2";
...
const HIGH_FREQUENCY_IDS = [
  "business_intro_wanlian",
  "main_product_experience_confirmed",
  "project_personal_contribution",
  "mvp_scope_business",
  "ai_agent_rag_policy",
  "park_role_understanding",
  "three_side_platform",
  "customization_productization",
  "data_dashboard",
  "policy_subsidy_flow",
  "supply_chain_finance_boundary",
  "weakness_no_park",
  "questions_to_ask",
];
const CATEGORY_GROUPS = [
  ...
  { label: "开场定位", ids: null, categories: ["开场与定位", "动机与匹配"] },
  { label: "核心项目", ids: null, categories: ["项目经历", "项目深挖"] },
  { label: "园区业务", ids: null, categories: ["岗位理解", "园区三端", "政策与补贴", "产业图谱", "数据驾驶舱", "供应链金融"] },
  { label: "AI与边界", ids: null, categories: ["AI与边界"] },
  { label: "半定制交付", ids: null, categories: ["半定制落地", "落地计划"] },
  { label: "高压速练", ids: null, categories: ["高压速练", "缺口防守"] },
  { label: "反问问题", ids: null, categories: ["反问问题"] },
...
const MODE_PRIORITY = {
  ...
  ai: [
    "ai_agent_rag_policy",
    "ai_enterprise_advisor",
    "ai_output_quality",
    "ai_bad_case_tuning",
    "main_product_experience_confirmed",
    "project_personal_contribution",
  ],
  delivery: ["customization_productization", "first_90_days", "policy_subsidy_flow", "data_dashboard"],
  pressure: [
    "weakness_no_park",
    "supply_chain_finance_boundary",
    "policy_subsidy_flow",
  ...
  mock: [
    "mock_q1_business_intro",
    "mock_q2_role_understanding",
    "mock_q3_project_story",
    "mock_q4_policy_flow",
    "mock_q5_ai_advisor",
    "mock_q6_customization",
    "mock_q7_weakness",
    "mock_q8_questions_to_ask",
  ],
  resume: [
...
function setView(view){
  ...
  $("pageTitle").textContent = isReview ? "集中复习" : isProduct ? "岗位理解" : "面试驾驶舱";
...
function exportMarkdown(){
  ...
  const markdown = ["# 万联易达园区版产品经理互动练习修改稿", "", "> 来源：interactive-prep 浏览器本地编辑导出，不覆盖 final-answers.md。", ""];
  for (const q of all) {
  ...
  downloadFile("wanlianyida-park-pm-edits.md", markdown.join("\n"), "text/markdown;charset=utf-8");
  downloadFile("wanlianyida-park-pm-edits.json", JSON.stringify(payload, null, 2), "application/json;charset=utf-8");
}
