...
AI is not a standalone chatbot here. It is placed inside the workflow to support diagnosis and suggestion generation. That experience is transferable to legal AI, because contract review also needs contract type, transaction background, review checklist, legal basis, risk points, suggested edits, and human review in a trustworthy workflow.`,          notes: "Use this when the interviewer may not understand facility operations. Start with the night alert scenario, then explain the product and your contribution.",
        ...
        zh: {
          ...
          followups: [
            ...
            {
              question: "你们现在用的是什么大模型？",
              intent: "防止被追问具体技术栈时夸大个人职责。",
              simple: "我会讲成可插拔模型接入，而不是绑定某一个模型。Advisor 或自然语言总结这类能力里，方案上有 GPT-4o / GPT-4.1-mini 这类云端模型接入；本地开发、演示或私有化部署也会考虑 Ollama + Qwen 这类本地模型或模板兜底。我参与的是产品侧方案和验收，不包装成我负责模型训练或底层调参。",
              full: "我了解的是，我们不是把产品能力绑定死在某一个模型上，而是按场景做可插拔接入。比如 Advisor、自然语言总结、诊断解释这类能力，可以接 GPT-4o / GPT-4.1-mini 这类云端模型；如果是本地开发、演示或者客户要求私有化部署，也会考虑 Ollama + Qwen 这类本地模型方案，或者在没有模型能力时使用模板兜底。\n\n但我不会说我是模型选型负责人。我的工作重点是定义模型在业务链路里要完成什么任务：比如解释告警、检索知识、生成建议、生成工单草稿；以及输出必须有哪些字段、什么时候低置信兜底、什么时候必须人工确认。具体模型由研发和算法同事结合部署环境、成本、合规和效果评估来选型。",
            },
            {
              question: "研发用什么搭建 Agent？你参与到什么程度？",
              intent: "追 Agent 工程架构和产品协作边界。",
              simple: "我会说研发侧是工具调用架构，不是简单聊天框。整体可以理解为前端 Vue、后端 Spring Boot、AI Engine FastAPI，Agent 层做工具注册、任务编排、业务 API 调用、权限控制和审计。我参与的是定义 Agent 能调用哪些工具、输入输出结构、失败提示、人审边界、结果回写和验收脚本。",
              full: "研发侧不是简单搭一个聊天框，而是按工具调用架构做的。整体上，前端是 Vue，后端是 Spring Boot，AI Engine 用 FastAPI；Agent 层会有工具注册、任务编排、业务 API 调用、权限控制和审计。用户问一个设备异常时，Agent 不是直接生成答案，而是先识别意图，再调用设备状态、传感器数据、告警、历史工单、SOP/知识库、仿真或预测等工具，拿到结果后再让模型生成结构化建议。\n\n我参与到产品协议这一层：Agent 能调用哪些工具、每个工具需要什么输入、返回什么结构、权限怎么控制、失败怎么提示、结果怎么回写、哪些高风险动作必须人工确认、怎么写验收脚本。研发负责具体编排和服务实现，我负责把业务语义和产品边界讲清楚。",
            },
            {
        ...
        en: {
          ...
          followups: [
            ...
            {
              question: "What LLM did your AI product use?",
              intent: "Keeps the model-stack answer accurate without overclaiming ownership.",
              simple: "I would frame it as a pluggable model setup rather than one fixed model. For Advisor or natural-language explanation, the design referenced GPT-4o / GPT-4.1-mini; for local demos or private deployment, Ollama plus Qwen-style local models or template fallback could be used. My ownership was product-side workflow and acceptance, not base-model training.",
              full: "My understanding is that the product was not permanently tied to one model. It was designed as pluggable by scenario. Advisor, natural-language explanation, and diagnostic summary could use cloud models such as GPT-4o or GPT-4.1-mini. For local development, demos, or private deployment, the team could use Ollama plus Qwen-style local models, or a template fallback when LLM capability was not available.\n\nI would not claim I owned model selection or model training. My focus was defining what the model should do inside the workflow: explain alerts, retrieve knowledge, generate suggestions, draft work orders, output structured fields, fall back on low confidence, and require human confirmation for high-risk actions.",
            },
            {
              question: "What did engineering use to build the Agent, and what was your role?",
              intent: "Checks engineering-awareness and product boundary.",
              simple: "I would describe it as a tool-calling architecture, not a chatbot. The stack can be understood as Vue frontend, Spring Boot backend, and FastAPI AI Engine. The Agent layer handles tool registry, task orchestration, business API calls, permission control, and audit. My role was to define tools, inputs, outputs, failure handling, human-review boundaries, writeback, and acceptance scripts.",
              full: "Engineering did not build it as a simple chat window. It was closer to a tool-calling architecture: Vue on the frontend, Spring Boot for backend services, and FastAPI for the AI Engine. The Agent layer handles tool registration, task orchestration, business API calls, permission control, and audit. When a user asks about an equipment anomaly, the Agent should not directly invent an answer. It first identifies intent, calls tools such as equipment status, sensor data, alerts, historical work orders, SOP or knowledge base, simulation, or prediction, and then uses the model to generate structured suggestions.\n\nMy role was at the product-contract layer: what tools the Agent can call, what inputs each tool needs, what output structure is expected, how permission is controlled, how failures are shown, how results write back, which high-risk actions need human confirmation, and how acceptance scripts are written. Engineering owned the orchestration and service implementation; I owned the business semantics and product boundaries.",
            },
            {
